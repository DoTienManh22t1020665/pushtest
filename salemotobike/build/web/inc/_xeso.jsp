...
<html lang="vi" data-cpath="${pageContext.request.contextPath}">
    ...
    <body>
        <div class="page">
            ...
            <c:set var="activeCat" value="${empty param.cat ? 'xeso' : param.cat}" />

            <header class="page-header">
                <div class="title-group">
                    <h1>Xe số</h1>
                    <div class="meta">
                        <span class="pill">Tổng số: <strong><c:out value="${fn:length(listXeSo)}"/></strong></span>
                    </div>
                </div>
                ...
                <nav class="cat-switch" aria-label="Chuyển danh mục xe">
                    ...
                    <a class="catbtn ${activeCat=='xeso' ? 'is-active' : ''}"
                       href="${cpath}/xeso"
                       <c:if test="${activeCat=='xeso'}">aria-current="page"</c:if>>Xe số</a>
            ...
            <c:choose>
                <c:when test="${not empty listXeSo}">
                    <section class="products-grid">
                        <c:forEach var="xe" items="${listXeSo}">
                ...
                <c:otherwise>
                    <div class="empty">
                        Chưa có dữ liệu xe số để hiển thị.
                    </div>
                </c:otherwise>
            </c:choose>
        </div>

...
<html lang="vi" data-cpath="${pageContext.request.contextPath}">
    ...
    <body>
        <div class="page">
            ...
            <c:set var="activeCat" value="${empty param.cat ? 'xemtatca' : param.cat}" />

            <header class="page-header">
                <div class="title-group">
                    <h1>Xem tất cả</h1>
                    <div class="meta">
                        <span class="pill">Tổng số: <strong><c:out value="${fn:length(listXe)}"/></strong></span>
                    </div>
                </div>
                ...
                <nav class="cat-switch" aria-label="Chuyển danh mục xe">
                    ...
                    <a class="catbtn ${activeCat=='xemtatca' ? 'is-active' : ''}"
                       href="${cpath}/xemtatca"
                       <c:if test="${activeCat=='xemtatca'}">aria-current="page"</c:if>>Xem tất cả</a>
                </nav>
            </header>

            <c:choose>
                <c:when test="${not empty listXe}">
                ...
                <c:otherwise>
                    <div class="empty">
                        Chưa có dữ liệu xe để hiển thị.
                    </div>
                </c:otherwise>
            </c:choose>
        </div>

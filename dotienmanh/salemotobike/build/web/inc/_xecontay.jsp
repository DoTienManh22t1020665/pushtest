...
<html lang="vi" data-cpath="${pageContext.request.contextPath}">
    ...
    <body>
        <div class="page">
            ...
            <c:set var="activeCat" value="${empty param.cat ? 'xecontay' : param.cat}" />

            <header class="page-header">
                <div class="title-group">
                    <h1>Xe côn tay</h1>
                    <div class="meta">
                        <span class="pill">Tổng số: <strong><c:out value="${fn:length(listXeConTay)}"/></strong></span>
                    </div>
                </div>
                ...
                <nav class="cat-switch" aria-label="Chuyển danh mục xe">
                    <a class="catbtn ${activeCat=='tayga' ? 'is-active' : ''}"
                       href="${cpath}/tayga">Xe tay ga</a>
                    <a class="catbtn ${activeCat=='xeso' ? 'is-active' : ''}"
                       href="${cpath}/xeso">Xe số</a>
                    <a class="catbtn ${activeCat=='xecontay' ? 'is-active' : ''}"
                       href="${cpath}/xecontay"
                       <c:if test="${activeCat=='xecontay'}">aria-current="page"</c:if>>Xe côn tay</a>
            ...
            <c:choose>
                <c:when test="${not empty listXeConTay}">
                    <section class="products-grid">
                        <c:forEach var="xe" items="${listXeConTay}">
                            <article class="product-card">
                                <div class="badge-wrap">
                                    <c:if test="${xe.xemoi}"><span class="badge-new">Mới</span></c:if>
                                </div>

                                <%-- Đánh dấu đã yêu thích dựa trên sessionScope.favIds --%>
                ...
                <c:otherwise>
                    <div class="empty">
                        Chưa có dữ liệu xe côn tay để hiển thị.
                    </div>
                </c:otherwise>
            </c:choose>
        </div>

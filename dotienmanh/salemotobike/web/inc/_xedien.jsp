...
<html lang="vi" data-cpath="${pageContext.request.contextPath}">
    ...
    <body>
        <div class="page">
            ...
            <c:set var="activeCat" value="${empty param.cat ? 'xedien' : param.cat}" />

            <header class="page-header">
                <div class="title-group">
                    <h1>Xe điện</h1>
                    <div class="meta">
                        <span class="pill">Tổng số: <strong><c:out value="${fn:length(listXeDien)}"/></strong></span>
                    </div>
                </div>
                ...
                <nav class="cat-switch" aria-label="Chuyển danh mục xe">
                    ...
                    <a class="catbtn ${activeCat=='xedien' ? 'is-active' : ''}"
                       href="${cpath}/xedien"
                       <c:if test="${activeCat=='xedien'}">aria-current="page"</c:if>>Xe điện</a>
            ...
            <c:choose>
                <c:when test="${not empty listXeDien}">
                    <section class="products-grid">
                        <c:forEach var="xe" items="${listXeDien}">
                ...
                <c:otherwise>
                    <div class="empty">
                        Chưa có dữ liệu xe điện để hiển thị.
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
